...
@interface ZSFTPManager ()

@property (nonatomic, readonly) uint8_t *         buffer;
@property (nonatomic, assign)   size_t            bufferOffset;
@property (nonatomic, assign)   size_t            bufferLimit;

- (void) _streamDidEndWithSuccess:(BOOL)success failureReason:(ZSFMStreamFailureReason)failureReason;
-(BOOL) _ftpActionForServer:(ZSFMServer*)server command:(NSString*)fullCommand;
@end

@implementation ZSFTPManager
...
- (BOOL) _checkFMServer:(ZSFMServer*)server {
    BOOL success = YES;
...
- (BOOL) _uploadData:(NSData *)data withFileName:(NSString *)fileName toServer:(ZSFMServer *)server {
    BOOL success = YES;
    
    action = _ZSFMCurrentActionUploadFile;
...
- (BOOL) _uploadFile:(NSURL*)fileURL toServer:(ZSFMServer*)server {
    BOOL success = YES;
...
- (BOOL) _createNewFolder:(NSString*)folderName atServer:(ZSFMServer*)server {
    BOOL success = YES;
    
    action = _ZSFMCurrentActionCreateNewFolder;
...
- (NSArray*) _contentsOfServer:(ZSFMServer*)server {
    BOOL success = YES;
    
    action = _ZSFMCurrentActionContentsOfServer;
...
- (BOOL) _downloadFile:(NSString*)fileName toDirectory:(NSURL*)directoryURL fromServer:(ZSFMServer*)server {
    BOOL success = YES;
    
    action = _ZSFMCurrentActionDownloadFile;
...
- (BOOL) uploadData:(NSData*)data withFileName:(NSString *)fileName toServer:(ZSFMServer*)server {
...
- (BOOL) uploadFile:(NSURL*)fileURL toServer:(ZSFMServer*)server {
    if (![self _checkFMServer:server]) {
        return NO;
    }
    if (!fileURL) {
...
- (BOOL) createNewFolder:(NSString*)folderName atServer:(ZSFMServer*)server {
    if (![self _checkFMServer:server]) {
        return NO;
    }
    if (!folderName) {
...
- (BOOL) deleteFileNamed:(NSString*)fileName fromServer:(ZSFMServer*)server {
    if (![self _checkFMServer:server]) {
        return NO;
    }
    if (!fileName) {
...
- (BOOL) deleteFile:(NSString *)absolutePath fromServer:(ZSFMServer *)server {
    //this is deprecated.
    //the method may not behave like it used to.
    return RunInSeparateThread([self deleteFileNamed:absolutePath fromServer:server]);
}
- (BOOL) chmodFileNamed:(NSString*)fileName to:(int)mode atServer:(ZSFMServer*)server {
    if (![self _checkFMServer:server]) {
        return NO;
    }
    if (!fileName) {
...
- (NSArray*) contentsOfServer:(ZSFMServer*)server {
    if (![self _checkFMServer:server]) {
        return nil;
    }
    return RunInSeparateThread([self _contentsOfServer:server]);
}
- (BOOL) downloadFile:(NSString*)fileName toDirectory:(NSURL*)directoryURL fromServer:(ZSFMServer*)server {
    if (![self _checkFMServer:server]) {
        return NO;
    }
    if (!fileName) {
...
- (BOOL) checkLogin:(ZSFMServer*)server {
    if (![self _checkFMServer:server]) {
        return NO;
    }
...
- (NSMutableDictionary *) progress {
    ...
    switch (action) {
        case _ZSFMCurrentActionUploadFile:
            currentStream = self.serverStream;
            break;
        case _ZSFMCurrentActionDownloadFile:
...
-(void)abort {
    ...
    switch (action) {
        ...
        case _ZSFMCurrentActionDownloadFile:
            currentStream = self.serverReadStream;
            break;
        case _ZSFMCurrentActionCreateNewFolder:
            currentStream = self.serverStream;
            break;
        case _ZSFMCurrentActionContentsOfServer:
            currentStream = self.serverReadStream;
            break;
        default:
            break;
    ...
    [self _streamDidEndWithSuccess:YES failureReason:ZSFMStreamFailureReasonAborted];
...
- (void) _streamDidEndWithSuccess:(BOOL)success failureReason:(ZSFMStreamFailureReason)failureReason {
    ...
    action = _ZSFMCurrentActionNone;
    streamSuccess = success;
    if (!streamSuccess) {
        switch (failureReason) {
            case ZSFMStreamFailureReasonReadError:
                NSLog(@"ftp stream failed: error while reading data");
                break;
            case ZSFMStreamFailureReasonWriteError:
                NSLog(@"ftp stream failed: error while writing data");
                break;
            case ZSFMStreamFailureReasonGeneralError:
...
- (void)stream:(NSStream *)theStream handleEvent:(NSStreamEvent)streamEvent {
    switch (streamEvent) {
        case NSStreamEventOpenCompleted:
            if (action == _ZSFMCurrentActionDownloadFile) {
        ...
        case NSStreamEventHasBytesAvailable:
            if (action == _ZSFMCurrentActionContentsOfServer) {
                NSInteger       bytesRead;
                
                bytesRead = [self.serverReadStream read:self.buffer maxLength:kSendBufferSize];
                if (bytesRead == -1) {
                    [self _streamDidEndWithSuccess:NO failureReason:ZSFMStreamFailureReasonReadError];
                } else if (bytesRead == 0) {
                    [self _streamDidEndWithSuccess:YES failureReason:ZSFMStreamFailureReasonNone];
                } else {
                    [self.directoryListingData appendBytes:self.buffer length:bytesRead];
                }
            } else if (action == _ZSFMCurrentActionDownloadFile) {
                ...
                if (self.bufferOffset != self.bufferLimit) {
                    ...
                    if (bytesWritten == -1 || bytesWritten == 0) {
                        [self _streamDidEndWithSuccess:NO failureReason:ZSFMStreamFailureReasonWriteError];
                    } else {
                ...
                [self _streamDidEndWithSuccess:NO failureReason:ZSFMStreamFailureReasonGeneralError];
            }
            break;
        case NSStreamEventHasSpaceAvailable:
            if (action == _ZSFMCurrentActionUploadFile) {
                if (self.bufferOffset == self.bufferLimit) {
                    //read process
        ...
        case NSStreamEventEndEncountered:
            if (action == _ZSFMCurrentActionCreateNewFolder) {
                [self _streamDidEndWithSuccess:YES failureReason:ZSFMStreamFailureReasonNone];
            }
            break;
        default:
...
-(BOOL) _ftpActionForServer:(ZSFMServer*)server command:(NSString*)fullCommand {
    ...
    action = _ZSFMCurrentActionSOCKET;
...
+(ZSFMServer*)serverWithDestination:(NSString*)dest username:(NSString*)user password:(NSString*)pass {
    ZSFMServer* server = [[ZSFMServer alloc] init];
    server.destination = dest;
    server.username = user;
    server.password = pass;
    return server;
}
+ (ZSFMServer*) anonymousServerWithDestination:(NSString*)dest {
    ZSFMServer* server = [[ZSFMServer alloc] init];
    server.destination = dest;
...
@implementation NSString (ZSFTPManagerNSStringAdditions)
